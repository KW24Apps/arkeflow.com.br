...
export async function authMiddleware(request: FastifyRequest, _reply: FastifyReply) {
  ...
  const [usuarioRes, cfgRes] = await Promise.all([
    platformPool.query<{ ultimo_acesso: Date | null; sessao_web: string | null; sessao_mobile: string | null }>(
      'SELECT ultimo_acesso, sessao_web, sessao_mobile FROM usuarios WHERE id = $1', [user.id]
    ),
