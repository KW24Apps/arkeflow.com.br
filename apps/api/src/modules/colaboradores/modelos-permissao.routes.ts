...
export async function modelosPermissaoRoutes(app: FastifyInstance) {
  ...
  app.get('/', { preHandler: dono }, async (req, reply) => {
    ...
    const { rows } = await platformPool.query(
      `SELECT * FROM modelos_permissao WHERE cliente_id = $1 ORDER BY nome`,
      [user.cliente_id]
    )
  ...
  app.post('/', { preHandler: dono }, async (req, reply) => {
    ...
    const { rows: [r] } = await platformPool.query(
      `INSERT INTO modelos_permissao (cliente_id, nome, permissoes)
       VALUES ($1, $2, $3) RETURNING *`,
      [user.cliente_id, data.nome, JSON.stringify(data.permissoes)]
    )
  ...
  app.put('/:id', { preHandler: dono }, async (req, reply) => {
    ...
    const { rows: [modelo] } = await platformPool.query(
      `SELECT sistema FROM modelos_permissao WHERE id = $1 AND cliente_id = $2`, [id, user.cliente_id]
    )
    ...
    const { rows: [r] } = await platformPool.query(
      `UPDATE modelos_permissao SET ${upd.join(', ')} WHERE id = $1 AND cliente_id = $2 RETURNING *`,
      val
    )
    return reply.send(r)
  })
  ...
  app.delete('/:id', { preHandler: dono }, async (req, reply) => {
    ...
    await platformPool.query(
      `DELETE FROM modelos_permissao WHERE id = $1 AND cliente_id = $2`, [id, user.cliente_id]
    )
